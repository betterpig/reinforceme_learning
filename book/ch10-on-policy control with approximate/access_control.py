...
def get_action(free_servers, priority, value_function):
    # if no free server, can't accept 没有空闲服务器
    if free_servers == 0:
        return REJECT
    if np.random.binomial(1, EPSILON) == 1:
        return np.random.choice(ACTIONS)    #随机
    values = [value_function.value(free_servers, priority, action) for action in ACTIONS]#这里又是只储存了value，但是索引刚好对应，0 1
...
def take_action(free_servers, priority, action):
    if free_servers > 0 and action == ACCEPT:
        free_servers -= 1   #接受则少1
    reward = REWARDS[priority] * action #若接受则reward是对应的reward，拒绝则reward为0
    # some busy servers may become free
    busy_servers = NUM_OF_SERVERS - free_servers
    free_servers += np.random.binomial(busy_servers, PROBABILITY_FREE)#二项分布，做正忙服务器数量次实验，每次实验成功的概率为free的概率
...
def figure_10_5():
    ...
    differential_semi_gradient_sarsa(value_function, max_steps) #非回合制，连续不终止问题，也就没有episode
    values = np.zeros((len(PRIORITIES), NUM_OF_SERVERS + 1))#优先级×空闲服务器数量
