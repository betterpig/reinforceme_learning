...
class Q_net(nn.Module):
    def __init__(self,in_features, hidden_features, out_features):
        ...
        self.layer_hidden = nn.Linear(in_features, hidden_features) # 输入层到隐藏层
        self.value = nn.Linear(hidden_features, 1)
        self.advantage = nn.Linear(hidden_features, out_features)#隐藏层到输出层
    def forward(self,x):#前向传播
        x = self.layer_hidden(x)
        x = t.relu(x)
        result=self.value(x)+(self.advantage(x)-t.mean(self.advantage(x)))
        return result
